...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const url = new URL(`/wp-json/wp/v2/terapeutas/${encodeURIComponent(id)}`, process.env.WORDPRESS_API_URL);
    ...
    const therapist = {
      ...
      email: data.meta?.email || data.acf?.correo_electronico || '',
      phone: data.meta?.phone || data.acf?.telefono || '',
      city: data.meta?.city || data.acf?.ciudad || '',
      department: data.meta?.department || data.acf?.departamento || '',
      experience_years: data.acf?.experiencia || '0',
      is_active: data.meta?.is_active ?? data.acf?.is_active ?? true,
    };
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const data = await wordpressAdminFetch(`/wp-json/wp/v2/terapeutas/${encodeURIComponent(id)}`, {
      method: "PUT",
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  try {
    await wordpressAdminFetch(`/wp-json/wp/v2/terapeutas/${encodeURIComponent(id)}?force=true`, {
      method: "DELETE",
    });
